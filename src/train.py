...
from src.config import TrainArgParser
...
def main():
    ...
    model, model_io = build_default_model(config, config['languages'], output_channels,
                                          path_to_saved_models=args.checkpoint_output)
    total_params = sum(p.numel() for p in model.kernel.parameters() if p.requires_grad)
