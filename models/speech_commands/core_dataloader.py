...
class SpeechCommandsDataset(SPEECHCOMMANDS):
    ...
    def __init__(self, path: str, subset: SpeechCommandsMode, predicate: Callable[[str], bool] = lambda label: True):
        ...
        if subset == SpeechCommandsMode.VALIDATION:
            ...
            self._walker = [w for w in self._walker if w not in excludes and predicate(w.split('/')[-2])]

    def load_list(self, filename: str, predicate: Callable[[str], bool] = lambda label: True) -> List[str]:
...
class SpeechCommandsBase(DataLoader, ABC):
    ...
    transform = torchaudio.transforms.Spectrogram(n_fft=97, hop_length=400)  # creates spectrograms of 1x49x40
...
class CoreDataLoader(SpeechCommandsBase):
    ...
    def label_to_index(self, word: str) -> int:
        ...
        if word in self.labels:
            ...
            return len(self.labels) - 1  # unknown
